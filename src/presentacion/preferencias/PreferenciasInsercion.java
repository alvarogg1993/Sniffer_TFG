...
//
/**
 * Permite seleccionar la estructura de un protocolo definido y generar un
 * paquete de datos con dicha estructura
 * 
 * @author Jose Manuel Saiz, Carlos Mardones Muga
 * @version 1.0
 * 
...
public class PreferenciasInsercion extends javax.swing.JDialog {
	...
	private static JPacket packete;
	private static String[] DispoName;
	private static PcapPacket p;
	private static JBuffer x;
	...
	private void initComponents() {
		...
		jcmbIpDcap.addFocusListener(new java.awt.event.FocusAdapter() {
			public void focusLost(ActionEvent evt) {
		...
		// java.awt.Font("Tahoma 11 Negrita 12 Negrita 12 Negrita 12 Simple", 0,
		// 12));
		...
		jTable1.setModel(new javax.swing.table.DefaultTableModel(
				new Object[][] { { null, null, null, null, null },
						...
						"Tamanho del campo", "Tipo de dato", "Opcional" }) {
		...
		jTable1.getColumnModel().getColumn(2)
				.setHeaderValue("Tamanho del campo");
		...
		jrMacOcap.addFocusListener(new java.awt.event.FocusAdapter() {
			public void focusGained(ActionEvent evt) {
				jrMacOcapFocusGained(evt);
			}
		});
...
//			}
	...
	public static void setFichero(String aux) {
		jtxtCapturas.setText(aux);
	}

	public String getFichero() {
		return jtxtCapturas.getText();
	}
	...
	/**
	 * Convierte valor definido como alfanumerico a su correspondiente valor en
	 * binario.
	 * 
	 * @param cadena
	 * @param tamanho
	 * @return string
	 */
	private String convertirAlfanumerico(String cadena, String tamanho) {
		String binario = "", aux, cero = "0";
		char letra;
		int tam = Integer.valueOf(tamanho);
	 ...
	 * @return string
	 */
	private String convertirBinario(String cadena, String tipo, String tamanho) {
		...
		if (tipo.equals("Booleano")) {
			return convertirBooleano(cadena, tamanho);
		}
		if (tipo.equals("Numerico")) {
			return convertirNumerico(cadena, tamanho);
		}
		if (tipo.equals("Alfanumerico")) {
			return convertirAlfanumerico(cadena, tamanho);
		}
		return null;
	}
	 ...
	 * @return string
	 */
	private String convertirNumerico(String cadena, String tamanho) {
		String binario, cero = "0";
		String aux = "";
		// solo recojo los caracteres que determina el tamanho
		int tam = Integer.valueOf(tamanho);
		...
		while (binario.length() != tam) {
			// si el tamanho no coincide con el maximo concateno 0 por la izq
			binario = cero.concat(binario);
		}
		return binario;
	}
	 ...
	 * @return string
	 */
	private String convertirBooleano(String cadena, String tamanho) {
		int tam = Integer.valueOf(tamanho);
		String aux = "";
...
//		p5 = packete.getHeader(new Arp());
//		p7= packete.getHeader(new Ip6());	
//		
		...
		if (!validar()) {
			...
			if (lista.length > 2) {
				...
				// try {
				// p2.setIPv6Parameter(10, 10, 10, 100, InetAddress
				// .getByAddress(convertirIp6(ipOr)), InetAddress
				// .getByAddress(convertirIp6(ipDes)));
				// } catch (UnknownHostException e) {
				// e.printStackTrace();
				// }
				// }
				...
				// try {
				// p3.setIPv6Parameter(10, 10, 10, 100, InetAddress
				// .getByAddress(convertirIp6(ipOr)), InetAddress
				// .getByAddress(convertirIp6(ipDes)));
				// } catch (UnknownHostException e) {
				// e.printStackTrace();
			...
			if (lista.length > 2) {
				if (lista[2].equals("ICMP")) {
					...
					// p3.typeEnum().TIMESTAMP_REQUEST_ID;
					// p3.typeEnum().TIMESTAMP_RESPONSE_ID;

					// p3.type() = Icmp.ICMP_TSTAMP;
					// p3.seq = 1000;
					// p3.id = 999;
			...
			if (definidos.size() != 0) {
				...
				if (p2 != null) {
					...
					// p1.data = datAux;
					p2.peerPayloadTo(new JBuffer(datAux));
				}
			} else {
				if (p1 != null) {
			...
			//
			// // open Jpcap
			// JpcapCaptor captor = JpcapCaptor.openDevice(device, 2000,
			// false, 3000);
			...
			// // Arp arp=new Arp();
			// p4.hardtype = Arp.HARDTYPE_ETHER;
			// p4.prototype = Arp.PROTOTYPE_IP;
			// p4.operation = Arp.ARP_REQUEST;
			// p4.hlen = 6;
			// p4.plen = 4;
			...
			// NetworkInterfaceAddress aux = new NetworkInterfaceAddress(ip
			// .getAddress(), masc.getAddress(), broad.getAddress(),
			// ipD.getAddress());
			//
			...
			// (byte) 255, (byte) 255, (byte) 255, (byte) 255 };
			// // Arp arp=new Arp();
			// p5.hardtype = Arp.HARDTYPE_ETHER;
			// p5.prototype = Arp.PROTOTYPE_IP;
			// p5.operation = Arp.ARP_REQUEST;
			// p5.hlen = 6;
			// p5.plen = 4;
			//
			// String[] mac = macOr.split(":");
...
//		System.out.println(p.toString());
	...
	/**
	 * Comprueba si los valores necesarios para la insercion estan correctamente
	 * creados.
	 * 
	 * @return boolean
	 */
	 ...
	 * @param evt
	 */
	private void jcmbIpDcapFocusLost(ActionEvent evt) {
	 ...
	 * @param evt
	 */
	private void jcmbIpOcapFocusLost(ActionEvent evt) {
	...
	private void jtxtIpDmanualFocusLost(FocusEvent evt) {
		ipDes = String.valueOf(jtxtIpDmanual.getText());
	}

	private void jtxtIp6origenFocusLost(FocusEvent evt) {
		ipOr = String.valueOf(jtxtIp6origen.getText());
	}

	private void jtxtIp6destinoFocusLost(FocusEvent evt) {
	 ...
	 */
	private byte[] recogerDatos(String cadena, String tipo, String tamanho) {
		String aux = "";
		char letra;
		int tam = Integer.valueOf(tamanho);
		...
		if (tipo.equals("Numerico")) {
			if (validarNumericos(cadena)) {
				...
				int res = tam / 8;
				if (res == 1) {
					byte[] auxB = new byte[] { (byte) pas.intValue() };
					return auxB;
				}
				if (res == 2) {
	...
	private void jrMacOcapFocusGained(ActionEvent evt) {
		jtxtMacOmanual.setEnabled(false);
	...
	private void jrMacDcapFocusGained(ActionEvent evt) {
		jtxtMacDmanual.setEnabled(false);
	 ...
	 * @param evt
	 */
	private void jcmbMacDcapFocusLost(ActionEvent evt) {
	 ...
	 * @param evt
	 */
	private void jcmbMacOcapFocusLost(ActionEvent evt) {
	...
	private static javax.swing.JTextField jtxtCapturas;
